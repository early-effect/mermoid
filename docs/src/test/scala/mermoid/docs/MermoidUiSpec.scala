...
/** Proves the bridge round-trips: the ascent UI tree, once server-rendered, carries the same structure mermoid's own
  * serializer produces. Tested directly rather than only through the doc pages, so a break points at this file.
  */
object MermoidUiSpec extends ZIOSpecDefault:
  ...
  private def render(mmd: String) = Html.render(MermoidUi.diagram(mmd))

  def spec = suite("MermoidUi")(
    suite("toUi")(
      ...
      test("text and raw both become text nodes") {
        assertTrue(
          MermoidUi.toUi(SvgNode.Text("hi")) == UI.Text("hi"),
          MermoidUi.toUi(SvgNode.Raw(".a { fill: red }")) == UI.Text(".a { fill: red }"),
        )
      },
    ),
    suite("parse failures")(
      test("an unparseable diagram throws rather than rendering an empty box") {
        assertTrue(scala.util.Try(MermoidUi.diagram("not a diagram at all")).isFailure)
      }
    ),
    suite("generated CSS is safe as a text node")(
      // toUi maps Raw -> UI.Text, and ascent escapes text nodes. That is only lossless while the CSS
      // contains no & < >. Assert it for every theme so a future selector like `.a > .b` fails here.
      test("no theme's stylesheet contains a character HTML escaping would rewrite") {
        val unsafe = for
          ...
          if !MermoidUi.cssIsEntitySafe(rendered)
    ...
    suite("SSR round-trip")(
      ...
      test("unresolved CSS variables also survive the round-trip") {
        // resolveVariables = false keeps `var(--mermoid-*)` in the output; that is the form a consumer
        // overrides from their own stylesheet, so it has to reach the page intact.
        for html <- Html.render(MermoidUi.diagram(flowchart, RenderConfig(resolveVariables = false)))
      ...
      test("every element mermoid emits reaches the HTML") {
        // Element count is the structural invariant: attribute quoting and self-closing style are
        // ascent's business, but a dropped or duplicated element would be a real bridge bug.
      ...
      test("a custom RenderConfig reaches the rendered output") {
        for
          ...
          dark    <- Html.render(MermoidUi.diagram(flowchart, RenderConfig(theme = css.ThemeName.Dark)))
